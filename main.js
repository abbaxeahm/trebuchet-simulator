...
var labelsForInput = ["lengthArmShort", "lengthArmLong", "pivotToArmCG", "lengthWeight", "lengthSling", "heightOfPivot", "massArm", "massWeight", "massProjectile", "inertiaArm", "inertiaWeight", "projectileDiameter", "windSpeed", "releaseAngle"]
var labelsForOutput = ["maxHeight", "maxDistance", "RangeEfficiency", "EnergyEfficiency", "ReleaseVelocity", "time"]
var inputValues = [1.85, 5.85, 0.85, 1, 5, 3.5, 80, 200, 2, 285, 80, 0.1, 0, 40]
var armCalc = false
let nArmParts = 0

function init () {
    for (i = 0; i < labelsForInput.length; i++)
        document.getElementById("div").innerHTML += labelsForInput[i] + `<input type="number" class="inputs" value="${inputValues[i]}"><input type="number" class="iInputs" value="0"><br>`
}

function test () {
    var csv = ""
    for (i in labelsForInput) {
        csv += labelsForInput[i] + ","
    }
    for (i in labelsForOutput) {
        csv += labelsForOutput[i] + ","
    }
    var inputs = document.getElementsByClassName("inputs")
    var VariableIndex = getRadioIndex(document.getElementsByName("check")) - 1
    var obj = {}
    var opj = {
        lengthArmShort: 1.85,
        lengthArmLong: 5.85,
        ...
        inertiaArm: 285,
        inertiaWeight: 80,
        projectileDiameter: 0.1,
        windSpeed: 0,
        CalculateDrag: true,
        releaseAngle: 40
    }
    for (i = 0; i < labelsForInput.length; i++) {
        obj[labelsForInput[i]] = parseFloat(inputs[i].value)
    }
    let mass = []
    let cgs = []
    let VariableIndexArm = []
    let arm = [[], [], [], []]
    for (i = 0; i < nArmParts; i++) {
        VariableIndexArm[i] = getRadioIndex(document.getElementsByName(`arm${i}`))
    }

    for (i = 0; i < parseInt(document.getElementById("i").value); i++) {
        for (j = 0; j < labelsForInput.length; j++) {
            obj[labelsForInput[j]] += parseFloat(document.getElementsByClassName("iInputs")[j].value)
        }
        if (armCalc) {
            // obj["lengthArmShort"] += parseFloat(document.getElementById("inc").value)
            // obj["lengthArmLong"] = parseFloat(document.getElementById("lengthArm").value) - obj["lengthArmShort"]
            // obj["pivotToArmCG"] -= parseFloat(document.getElementById("inc").value)
            let cg = 0
            let massSum = 0
            let massDSum = 0
            let lengthArm = 0
            obj["massArm"] = 0
            obj["inertiaArm"] = 0
            for (j = 0; j < nArmParts; j++) {
                arm[0][j] = parseFloat(document.getElementsByClassName("density")[j].value)
                arm[1][j] = parseFloat(document.getElementsByClassName("length")[j].value)
                arm[2][j] = parseFloat(document.getElementsByClassName("width")[j].value)
                arm[3][j] = parseFloat(document.getElementsByClassName("height")[j].value)
                mass[j] = 1
                for (k = 0; k < 4; k++) {
                    arm[k][j] += parseFloat(document.getElementById("iArm" + String(j) + String(k)).value) * i
                    mass[j] *= arm[k][j]
                }
                cgs[j] = arm[1][j] / 2
                for (k = 1; k <= j; k++) {
                ...
                massDSum += cgs[j] * mass[j]
                massSum += mass[j]
                obj["massArm"] += mass[j]
                lengthArm += arm[1][j]
            }
            cg = massDSum / massSum
            obj["pivotToArmCG"] = cg - obj["lengthArmShort"]
            obj["lengthArmLong"] = lengthArm - obj["lengthArmShort"]
            for (j = 0; j < nArmParts; j++) {
                obj["inertiaArm"] += mass[j] * ((arm[1][j] ** 2 + arm[3][j] ** 2) / 12 + (Math.abs(cgs[j] - cg) ** 2))
            }
            if (typeof obj["inertiaArm"] === NaN) {
                obj["inertiaArm"] = 200
            }
        }
        if (!isNaN(obj["lengthArmShort"])) {
            var result = simulator(obj)
            obj.maxHeight = result.MinMax[1][1]
            obj.maxDistance = result.MinMax[1][0]
            obj.RangeEfficiency = result.RangeEfficiency
            obj.EnergyEfficiency = result.EnergyEfficiency
            obj.ReleaseVelocity = result.ReleaseVelocity
            obj.time = result.Time[result.Time.length - 1]
            console.log(obj)
            csv += "\n"

            for (j in labelsForInput) {
                csv += obj[labelsForInput[j]] + ","
            }
            for (j in labelsForOutput) {
                csv += obj[labelsForOutput[j]] + ","
            }
        }
    }
    csv = csv.replaceAll(",", ";")
...
function getRadioIndex (elements) {
...
function addArmPart () {
    let density = "700"
    if (nArmParts) {
        density = document.getElementsByClassName("density")[0].value
    }
    document.getElementById("armParts").innerHTML += `density(kg/m3)<input type="number" class="density" value="${density}"><input type="number" id="iArm${nArmParts}0" value="0"> length(m) <input type="number" class="length" value="4"><input type="number" id="iArm${nArmParts}1" value="0">width(m) <input type="number" class="width" value="0.095"><input type="number" id="iArm${nArmParts}2" value="0">height(m)<input type="number" class="height" value="0.045"><input type="number" id="iArm${nArmParts}3" value="0"><br>`
    nArmParts++
}

function armCalcSwitch () {
    armCalc = !armCalc

    if (armCalc) {
        document.querySelector("button[onclick='armCalcSwitch()']").innerHTML = "disable"
    } else {
        document.querySelector("button[onclick='armCalcSwitch()']").innerHTML = "enable"
    }
}
